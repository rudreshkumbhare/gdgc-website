...
// Individual magnetic card wrapper component
function TeamCard({ member }) {
  const { cardRef, glowRef } = useMagneticHover({ strength: 0.28, tiltStrength: 10, lift: true })

  return (
    <article ref={cardRef} className={`team__card team__card--${member.color} animate-fadeUp`}>
      {/* Cursor-following glow layer */}
      <div ref={glowRef} className="team__card-glow" aria-hidden="true" />
      ...
      <div className="container">
        ...
        <div className="team__grid stagger">
          {TEAM_MEMBERS.map((member) => (
            <ScrollReveal key={member.name} as="div" baseOpacity={0.2} blurStrength={6}>
